{ bc, stdenv, writeScriptBin }:

writeScriptBin "scp-speed-test" ''
#!${stdenv.shell}
...
`dd if=/dev/zero of=$test_file bs=$(echo "$test_size*1024" | ${bc}/bin/bc) \
count=1 &> /dev/null`
...
up_speed=`echo "($up_speed*0.0009765625*100.0+0.5)/1*0.01" | ${bc}/bin/bc`
...
down_speed=`echo "($down_speed*0.0009765625*100.0+0.5)/1*0.01" | ${bc}/bin/bc`
